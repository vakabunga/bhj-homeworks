class Autocomplete {
  ...
  getMatches(text) {
    let matchElems = [];
    const selectElems = Array.from(this.input.children);
    for (let selectElem of selectElems.map(function(elem) {
      return { text: elem.text, value: elem.value };
    })) {
      if (selectElem.text.includes(text)) matchElems.push(selectElem);
    }
    return matchElems;
  }
}
