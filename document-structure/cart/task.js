...
function changeQuantity(e) {
  // выделяем кнопки уменьшения количества и уменьшаем количество товара при нажатии на 1, если количество больше 1
  if (e.target.classList.contains('product__quantity-control_dec')) {
    if (Number(e.target.nextElementSibling.textContent) > 1) e.target.nextElementSibling.textContent--;
  }
  // выделяем кнопки увеличения количества и увеличиваем при нажатии количество товара на 1
  if (e.target.classList.contains('product__quantity-control_inc')) e.target.previousElementSibling.textContent++;
}
...
function addToCart(e) {
  ...
  // если в корзине элементов нет, отображаем контейнер с корзиной
  if (cartProduct.length === 0) document.querySelector('.cart').removeAttribute('style');
  // если что-то есть, то проверяем корзину на наличие элементов
